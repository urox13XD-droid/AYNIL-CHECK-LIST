...
export function ImportPanel({
  ...
  const readFile = useCallback(
    (file: File) => {
      ...
      reader.onload = () => {
        const text = extractTextFromFile(file.name, String(reader.result));
    ...
    <aside className="no-print flex h-full w-96 shrink-0 flex-col border-r-[3px] border-black bg-white">
      <div className="border-b-[3px] border-black p-3">
        ...
        <div
          ...
          className={`rounded-lg border-[2px] border-dashed p-1 transition ${
        ...
        >
          <textarea
            ...
            placeholder={"Collez la liste du loueur ici, ou glissez un fichier .txt / .csv…\n\nEx.\n1x ARRI Alexa 35\n2x Teradek Bolt 6 TX\nSmallHD Ultra 5"}
            rows={9}
            className="w-full resize-none rounded-md border-0 bg-transparent p-2 text-xs font-semibold outline-none placeholder:font-normal placeholder:text-black/40"
          />
        </div>
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          <ComicButton onClick={onAnalyze} disabled={!rawText.trim()}>
            Analyser
          </ComicButton>
        </div>
      </div>
      ...
      <div className="border-t-[3px] border-black p-3 text-[10px] font-semibold leading-relaxed text-black/60">
        Formats supportés : texte collé ou fichier .txt / .csv. L’analyse de PDF/Excel du loueur
        est prévue via une passe IA de reconnaissance en amont.
      </div>
    </aside>
  );
}
